...
Point the cache at **shared research space:**
```bash
...
2. **Accept the model license** on the [ESMFold2 model page](https://huggingface.co/biohub/ESMFold2) — this is required before you can download the weights, even with a valid token. Do this before generating a token below.
3. Create a token at [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens). **Read** permission is sufficient — ESMFold2 only downloads weights, it never writes back to Hugging Face.
4. **Copy and save** your token after creating it (format: `hf_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX`)
5. Log in:
   ```bash
   hf auth login
   ```
...
| Column | Description |
|---|---|
| `interaction_id` | Label for this interaction in output files |
...
- `HF_HOME` — should point to your shared research-space cache (see Step 2)
...
- One `.cif` structure file per sample
- A `summary.csv` recording `plddt_mean`, `ptm`, and `iptm` for every structure generated

**ESMFold2 ranks structures by ipTM score alone** when selecting the best prediction per interaction.
...
```
This copies the winning `.cif` file per interaction into your chosen output directory. Along with this `summary_scores.csv` gives summary scores for each of the highest ranking samples. Add `--dry-run` to preview what would be copied without actually copying anything.
